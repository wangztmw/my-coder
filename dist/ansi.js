 ...
 */
const B = '\x1b[1m';
const b = '\x1b[22m'; // bold
const D = '\x1b[2m';
const d = '\x1b[22m'; // dim
const G = '\x1b[90m';
const c = '\x1b[39m'; // gray → reset
export function mdToANSI(text) {
    // 非TTY(管道/重定向) → 纯文本
    if (!process.stdout.isTTY)
        return text.replace(/```[\s\S]*?```/g, '[code]').replace(/[*#`|>-]/g, '');
    ...
    result = result.replace(/^\|(.+)\|$/gm, (_, row) => {
        ...
        return '  ' + cells.map((s) => s.padEnd(20)).join(' ').trim();
    });
    // 水平线
    ...
    let bOpen = 0, dOpen = 0;
    const re = /\x1b\[(1|22|2)m/g;
    let m;
    while ((m = re.exec(result)) !== null) {
        if (m[1] === '1')
            bOpen++;
        else if (m[1] === '22')
            bOpen = Math.max(0, bOpen - 1);
        else if (m[1] === '2')
            dOpen++;
        else if (m[1] === '22')
            dOpen = Math.max(0, dOpen - 1);
    }
    if (bOpen > 0)
        result += b.repeat(bOpen);
    if (dOpen > 0)
        result += d.repeat(dOpen);
    // 去掉残留的未配对*号

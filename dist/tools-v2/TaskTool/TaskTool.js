...
import { readMemberOutput } from '../../team.js';
...
function fmtTask(t) {
    ...
    if (t.agentLoop && t.status === 'running') {
        ...
        if (t.agentLoop.lastActivity)
            line += `, last: ${t.agentLoop.lastActivity}`;
    }
...
export const TaskTool = buildTool({
    ...
    isReadOnly: () => false,
    async call({ action, taskId, timeout_ms, instruction }, _ctx) {
        if (!_tasks || !_pendingNotifs)
        ...
        switch (action) {
            ...
            case 'direct': {
                if (!taskId)
                    return { data: 'Error: taskId required' };
                if (!instruction)
                    return { data: 'Error: instruction required' };
                const dt = _tasks.get(taskId);
                if (!dt)
                    return { data: `Task ${taskId} not found.` };
                if (dt.status !== 'running')
                    return { data: `Task ${taskId} is ${dt.status}, cannot inject instruction.` };
    ...
    userFacingName: () => 'Task',
    getToolUseSummary({ action, taskId }) {

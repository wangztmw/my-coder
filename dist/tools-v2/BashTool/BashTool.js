...
let _bgHooks = null;
export function initBashBg(hooks) { _bgHooks = hooks; }
// ============================================================
// Phase 46: Agent 自保机制
// ============================================================
// 自 PID 感知
...
function targetsSelf(command) {
    const killVerbs = /\b(kill|pkill|killall|SIGTERM|SIGKILL)\b/i;
    if (!killVerbs.test(command))
        return false;
    const patterns = SELF_IDENTITY
        .map(id => { try {
        return new RegExp(`\\b${id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
    }
    catch {
        return null;
    } })
...
export const BashTool = buildTool({
    ...
    isReadOnly: () => false,
    async call({ command, description, timeout, run_in_background }, _ctx) {

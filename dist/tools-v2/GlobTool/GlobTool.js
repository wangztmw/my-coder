...
export const GlobTool = buildTool({
    ...
    async call({ pattern, path }, _ctx) {
        const dir = path || '.';
        try {
            // Try ripgrep first (faster + .gitignore-aware), fall back to find
            let stdout;
            try {
                stdout = execSync(`rg --files -g '${pattern.replace(/'/g, "'\\''")}' '${dir.replace(/'/g, "'\\''")}' 2>/dev/null`, { encoding: 'utf-8', timeout: 10000, maxBuffer: 1024 * 1024 });
            }
            catch {
                stdout = execSync(`find '${dir.replace(/'/g, "'\\''")}' -path '${pattern.replace(/'/g, "'\\''")}' -not -path '*/node_modules/*' -not -path '*/.git/*' -maxdepth 8 2>/dev/null`, { encoding: 'utf-8', timeout: 10000, maxBuffer: 1024 * 1024 });
            }

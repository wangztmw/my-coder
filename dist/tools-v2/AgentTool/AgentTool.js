...
const agentTreeLock = (() => {
    try {
        return require('../../task_tree/lock.js').sharedLock;
    }
    catch {
        return new TreeWriteLock();
    }
})();
...
async function syncTreeNode(task, status, result) {
        ...
        await agentTreeLock.batch(task.id, async () => {
            ...
            if (tree) {
                ...
                try {
                    ...
                    appendWal(tree.sessionId, task.treeNodeId, status === 'completed' ? 'node_completed' : 'node_failed', { result: result.slice(0, 500) });
                }
                catch { }
            }
        });
...
export const AgentTool = buildTool({
    ...
    async call({ description, prompt, subagent_type: _type, run_in_background, context_files, parent_depth, parent_node_id }, _ctx) {
            ...
            try {
                await agentTreeLock.batch(task.id, async () => {
                    ...
                    if (tree) {
                        ...
                        if (child) {
                            task.treeNodeId = child.id;
                            task.treeRole = child.role;
                            saveTree(tree);
                        }
                    }
                });
        ...
        if (context_files && context_files.length > 0) {
            ...
            for (const [id, t] of _tasks) {
                ...
                if (!t.contextFiles || t.contextFiles.length === 0)
                    continue;
                const intersection = context_files.filter((f) => t.contextFiles.includes(f));
                if (intersection.length > 0)
                    conflicts.push(`${id} ("${t.subject}"): ${intersection.join(', ')}`);
            }
            if (conflicts.length > 0) {
                task.status = 'blocked';
                task.feedback = `File conflict: ${conflicts.join('; ')}`;
            ...
            try {
                ...
                if (!lr.ok) {
                    task.status = 'blocked';
                    task.feedback = `File lock: ${lr.conflictFile} held by ${lr.heldBy}`;
                    task.feedbackAt = Date.now();
        ...
        const subConfig = {
            ...
            systemPrompt: childDepth >= 2 ? _engine.buildSystemPrompt?.('worker')
                : childDepth === 1 ? _engine.buildSystemPrompt?.('supervisor')
                    : undefined,
            fileTracker: undefined, // TODO Phase 6b: pass createFileTrackerHook(task.id, task.treeNodeId)
            onComplete: (text) => { },
            preRoundCheck: () => {
                ...
                if (task.treeNodeId && _engine?.activeTreeId) {
                    try {
                        ...
                        if (tree && !isAncestorAlive(tree, task.treeNodeId, {
                            ...
                            getMemberStatus: (agentId) => _tasks?.get(agentId)?.status,
        ...
        if (run_in_background) {
            ...
            if (bgNodeId && bgTreeId) {
                try {
                    await agentTreeLock.batch(task.id, async () => {
                        ...
                        if (tree) {
                            dispatchNode(tree, bgNodeId, task.id);
                            appendWal(bgTreeId, bgNodeId, 'node_dispatched', { agentId: task.id });
                            saveTree(tree);
                        }
                    });
                }
            ...
            agentLoop(_engine, subConfig).then(result => {
                if (result.status === 'success') {
                    ...
                    if (t) {
                        t.status = result.status === 'blocked' ? 'blocked' : 'failed';
                        t.endTime = Date.now();
                        t.output = `[${result.status}] ${result.text}`;
                        if (result.blockedReason)
                            t.feedback = result.blockedReason;
                    }
                ...
                syncTreeNode(task, 'failed', err.message);
                import('../../task_tree/file_tracker.js').then(m => m.releaseFileLocks(task.id)).catch(() => { });
        ...
        if (syncNodeId && syncTreeId) {
            try {
                await agentTreeLock.batch(task.id, async () => {
                    ...
                    if (tree) {
                        dispatchNode(tree, syncNodeId, task.id);
                        appendWal(syncTreeId, syncNodeId, 'node_dispatched', { agentId: task.id });
                        saveTree(tree);
                    }
                });
            }
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (result.blockedReason)
                    task.feedback = result.blockedReason;
                syncTreeNode(task, result.status === 'blocked' ? 'blocked' : 'failed', result.text);
            }
            import('../../task_tree/file_tracker.js').then(m => m.releaseFileLocks(task.id)).catch(() => { });
            ...
            syncTreeNode(task, 'failed', e.message);
            import('../../task_tree/file_tracker.js').then(m => m.releaseFileLocks(task.id)).catch(() => { });

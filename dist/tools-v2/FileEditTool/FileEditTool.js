...
export const FileEditTool = buildTool({
    ...
    isReadOnly: () => false,
    async call({ file_path, old_string, new_string, replace_all }, _ctx) {
        ...
        try {
            ...
            return { data: `Error reading ${file_path}: ${e.message}` };
        }
        // CRLF → LF 规范化
        ...
        // 执行替换
        const result = normalizeLineEndings(replace_all ? normalized.split(searchStr).join(new_string) : normalized.slice(0, positions[0]) + new_string + normalized.slice(positions[0] + searchStr.length), content);
        ...
        try {
            ...
            return { data: `Error writing ${file_path}: ${e.message}` };
        }
    ...
    userFacingName: () => 'Edit',
    getToolUseSummary({ file_path: fp, old_string: old }) {
        return fp ? `Edit: ${fp}${old ? ` "${old.slice(0, 40)}"` : ''}` : null;
    },
});
// 保持原始换行符风格
function normalizeLineEndings(result, original) {

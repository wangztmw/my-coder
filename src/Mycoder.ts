...
import { lockSession, unlockSession, hasUnfinishedSession, loadSession, saveSession } from './session.js';
...
async function main() {
  ...
  try {
    ...
    setMemberGetter((id: string) => teamReg.get(id));
  ...
  if (shouldResume && hasUnfinishedSession()) {
    ...
    if (session) {
      ...
      engine.sessionMessages = session.messages as ChatMessage[];
      ...
      try {
        ...
        const result = resumeSessionOrchestrator(oldTreeId, (id: string) => teamReg.get(id));
  ...
  // 每次 agent 完成一轮后保存会话
  engine.onTurnComplete = (messages: typeof engine.sessionMessages, tc: number) => {
    toolCount += tc;
  ...
  initBashBg({
    createTask: addMember as (t: string, d: string) => unknown,
